...
import { 
  Trash2, MessageSquare, Search, Plus, X, LayoutGrid, Table as TableIcon, 
  HelpCircle, Calendar, User, Package, ExternalLink, CheckCircle, 
  CheckSquare, Square, Reply, Send, Filter, MoreHorizontal
} from 'lucide-react';
...
function QuestionsContent() {
  ...
  const handleDeleteQuestion = async () => {
    ...
    try {
      ...
      const response = await fetch(`/api/products/${question.productId}/questions/${id}`, {
        method: 'DELETE',
      });
  ...
  const handleBulkDelete = async () => {
    ...
    try {
      ...
      await Promise.all(questionsToDelete.map(q => 
        fetch(`/api/products/${q.productId}/questions/${q.id}`, { method: 'DELETE' })
      ));
  ...
  const handleAddAnswer = async (question: ProductQuestion) => {
    ...
    try {
      ...
      const response = await fetch(`/api/products/${question.productId}/questions/${question.id}/answers`, {
        method: 'POST',
  ...
  const handleDeleteAnswer = async () => {
    ...
    try {
        ...
        if (selectedQuestion?.id === questionId) {
            setSelectedQuestion(prev => prev ? { ...prev, answers: prev.answers.filter(a => a.id !== answerId) } : null);
        }
        setConfirmDialog(prev => ({ ...prev, isOpen: false }));

        await fetch(`/api/products/${productId}/questions/${questionId}/answers/${answerId}`, {
            method: 'DELETE',
        });
  ...
  return (
    <PageContainer className="max-w-[1600px] px-6 py-12 relative">
      ...
      <div className="flex flex-col md:flex-row md:items-end justify-between gap-6 mb-12">
        <div>
          <h1 className="text-3xl font-light tracking-tight text-neutral-900">Q&A</h1>
          <p className="text-neutral-500 font-light mt-2">Manage customer questions and provide answers.</p>
        </div>
        <div className="flex items-center gap-3">
          <BinButton itemType="answer" title="Answers Bin" filterByQuestionId={true} onRestore={fetchData} />
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {[
          { label: 'Total Questions', value: stats.total, icon: HelpCircle, filter: 'all' },
          { label: 'Unanswered', value: stats.unanswered, icon: MessageSquare, filter: 'unanswered', alert: stats.unanswered > 0 },
          { label: 'Answered', value: stats.answered, icon: CheckCircle, filter: 'answered' },
          { label: 'Total Replies', value: stats.totalAnswers, icon: Reply, filter: 'all' },
        ].map((stat, i) => (
          <button 
            key={i}
            onClick={() => stat.filter !== 'all' && setFilterStatus(stat.filter as any)}
          ...
          >
            <div className="flex items-center justify-between mb-2">
              <p className={`text-xs font-medium uppercase tracking-wider ${stat.alert ? 'text-black/60' : 'text-neutral-500'}`}>{stat.label}</p>
              <stat.icon className={`w-4 h-4 ${stat.alert ? 'text-black/55' : 'text-neutral-400'}`} />
            </div>
            <p className={`text-2xl font-light ${stat.alert ? 'text-black' : 'text-neutral-900'}`}>{stat.value}</p>
          </button>
        ))}
      </div>
      ...
      {filteredQuestions.length === 0 ? (
        <SectionCard>
          <div className="text-center py-16">
            ...
            <h3 className="text-lg font-medium text-neutral-900 mb-1">No Questions Found</h3>
              ...
              >
                ...
                <div className="flex items-start justify-between mb-4 pr-8">
                  <div className="flex items-center gap-3">
                    ...
                    <div>
                       <p className="text-sm font-medium text-neutral-900">{question.userName}</p>
                       <p className="text-xs text-neutral-400">{formatDistanceToNow(new Date(question.createdAt), { addSuffix: true })}</p>
                    </div>
                  </div>
                </div>
                ...
                <div className="flex-1 mb-4">
                  <div className="flex items-start gap-2 mb-2">
                      ...
                      <h3 className="font-medium text-neutral-900 line-clamp-3 leading-relaxed">{question.question}</h3>
                  </div>
              ...
              <tr>
                <th className="px-6 py-4 w-12">
                   <button onClick={toggleSelectAll} className="text-neutral-400 hover:text-neutral-900">
                     {selectedIds.size === filteredQuestions.length && filteredQuestions.length > 0 ? <CheckSquare className="w-4 h-4" /> : <Square className="w-4 h-4" />}
                   </button>
                </th>
                <th className="px-6 py-4 text-xs font-medium text-neutral-500 uppercase tracking-wider">User</th>
                <th className="px-6 py-4 text-xs font-medium text-neutral-500 uppercase tracking-wider">Question</th>
                  ...
                  return (
                    ...
                    >
                       <td className="px-6 py-4" onClick={(e) => e.stopPropagation()}>
                          <button onClick={(e) => toggleSelect(question.id, e)} className="text-neutral-300 hover:text-neutral-900">
                       ...
                       <td className="px-6 py-4">
                          <div className="flex items-center gap-2">
                             ...
                             <span className="text-sm font-medium text-neutral-900">{question.userName}</span>
                          </div>
                       </td>
                       <td className="px-6 py-4 max-w-xs">
                          <p className="text-sm text-neutral-600 truncate">{question.question}</p>
                       </td>
                       <td className="px-6 py-4">
                          {isUnanswered ? (
                       ...
                       <td className="px-6 py-4 whitespace-nowrap">
                          <span className="text-sm text-neutral-500">{format(new Date(question.createdAt), 'MMM d, yyyy')}</span>
                       </td>
                       <td className="px-6 py-4 text-right" onClick={(e) => e.stopPropagation()}>
                          <button 
                             onClick={() => setConfirmDialog({ isOpen: true, type: 'delete', itemId: question.id })}
         ...
         <div className="fixed inset-0 z-50 flex items-center justify-center p-4 sm:p-6">
            ...
            <div className="relative w-full max-w-4xl bg-white shadow-2xl shadow-neutral-900/20 rounded-[2rem] flex flex-col max-h-[90vh] animate-in zoom-in-95 slide-in-from-bottom-4 duration-300 overflow-hidden ring-1 ring-black/5">
               ...
               <div className="px-8 py-6 border-b border-neutral-100 flex items-center justify-between bg-white/80 backdrop-blur-xl z-10 shrink-0">
                  <div className="flex items-center gap-4">
                     <div className="w-12 h-12 rounded-full bg-neutral-100 flex items-center justify-center border border-neutral-200">
                        <span className="text-xl font-light text-neutral-600">{selectedQuestion.userName.charAt(0).toUpperCase()}</span>
                     </div>
                     <div>
                        <h2 className="text-xl font-medium text-neutral-900">{selectedQuestion.userName}</h2>
                        <div className="flex items-center gap-2 text-sm text-neutral-500">
                           <span>{format(new Date(selectedQuestion.createdAt), 'MMMM d, yyyy')}</span>
                           <span className="text-neutral-300">•</span>
                           <span>{format(new Date(selectedQuestion.createdAt), 'h:mm a')}</span>
                        </div>
                     </div>
                  </div>
                  <button 
                     ...
                     <div className="lg:col-span-2 p-8 space-y-8">
                        ...
                        <div className="space-y-4">
                           <div className="flex items-center gap-2">
                              ...
                              <span className="text-sm font-bold uppercase tracking-wider text-neutral-900">Question</span>
                           </div>
                           
                           <div className="bg-white p-6 rounded-2xl border border-neutral-100 shadow-sm">
                              <p className="text-lg text-neutral-900 leading-relaxed font-light">{selectedQuestion.question}</p>
                           </div>
                        </div>
                           ...
                           <div className="space-y-4">
                              {selectedQuestion.answers?.map(answer => (
                                 <div key={answer.id} className="bg-neutral-100/50 rounded-2xl p-5 border border-neutral-100 relative group">
                                    <div className="flex items-center justify-between mb-2">
                                       ...
                                       <button 
                                          onClick={() => setConfirmDialog({ 
                                             ...
                                             type: 'delete-answer', 
                                             itemId: null, 
                                             meta: { questionId: selectedQuestion.id, answerId: answer.id, productId: selectedQuestion.productId }
                                          })}
                                    ...
                                    <p className="text-sm text-neutral-600 leading-relaxed">{answer.answer}</p>
                                 </div>
                              ))}
                           </div>
               ...
               <div className="px-8 py-5 border-t border-neutral-100 bg-white flex justify-between items-center z-10 shrink-0">
                  <span className="text-xs text-neutral-400 font-mono">ID: {selectedQuestion.id}</span>
                  <button 
                     onClick={() => setConfirmDialog({ isOpen: true, type: 'delete', itemId: selectedQuestion.id })}
      ...
      <ConfirmDialog
        ...
        title={confirmDialog.type === 'delete-answer' ? "Delete Answer" : "Delete Question"}
