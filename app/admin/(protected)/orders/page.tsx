  ...
  const deleteOrder = async () => {
    if (deleteConfirm.isBulk) {
      ...
      try {
        ...
        const res = await fetch(`/api/orders/${deleteConfirm.orderId}`, {
          method: "DELETE",
        });
  ...
  return (
    <PageContainer className="max-w-[1600px] px-6 py-12 relative">
      ...
      {selectedIds.size > 0 && (
        <div className="fixed bottom-6 left-1/2 transform -translate-x-1/2 bg-white border border-neutral-200 shadow-2xl rounded-full px-6 py-3 flex items-center gap-4 z-40 animate-in slide-in-from-bottom-6 fade-in">
          ...
          <div className="flex items-center gap-2">
            ...
            <button
              onClick={() =>
                setDeleteConfirm({ isOpen: true, orderId: null, isBulk: true })
              }
              className="p-2 text-neutral-400 hover:text-black/55 rounded-lg transition-colors"
            >
                      ...
                      <div>
                        ...
                        <div className="flex items-baseline gap-2">
                          <span className="text-4xl font-light text-neutral-900">
                            {format(new Date(selectedOrder.pickupDate), "d")}
                          </span>
                          <span className="text-xl text-neutral-400">
                            {format(new Date(selectedOrder.pickupDate), "MMM")}
                          </span>
                        </div>
                    ...
                    <div className="flex items-center gap-3">
                      ...
                      <div>
                        ...
                        <p className="text-xs text-neutral-500 font-medium">
                          {selectedOrder.items.length} items in cart
                        </p>
                      </div>
                    </div>
                  </div>
      ...
      <ConfirmDialog
        ...
        onCancel={() =>
          setDeleteConfirm({ isOpen: false, orderId: null, isBulk: false })
        }
      />
    </PageContainer>
  );
