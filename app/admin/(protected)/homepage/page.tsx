...
export default function HomepagePageContent() {
    ...
    try {
      ...
      if (res.ok) {
        ...
        const validSlides = data.filter(
          (slide: BannerSlide) => slide && slide.id,
        );
        setBannerSlides(
          validSlides.sort(
            (a: BannerSlide, b: BannerSlide) => a.displayOrder - b.displayOrder,
          ),
        );
      }
    } catch (error) {
  ...
  return (
    <AccessControl requiredPermission="cms:banners">
      <PageContainer className="max-w-[1200px] px-6 py-12">
        <div className="mb-10">
          ...
          <p className="text-neutral-500 font-light mt-2">
            Manage your homepage hero slider.
          </p>
        </div>
            ...
            <div className="space-y-4">
              {bannerSlides.map((slide, index) => (
                ...
                >
                  {/* Preview */}
                  <div
                    className={`w-full sm:w-48 h-28 shrink-0 rounded-xl overflow-hidden relative ${!slide.imageUrl ? slide.bgColor : ""}`}
                  >
                    {slide.imageUrl ? (
                      <Image
                        src={slide.imageUrl}
                      ...
                      <div
                        className={`w-full h-full flex items-center justify-center ${slide.textColor}`}
                      >
