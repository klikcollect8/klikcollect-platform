...
function ReviewsContent() {
  ...
  const handleDeleteReview = async () => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/products/${review.productId}/reviews/${id}`,
        {
          method: "DELETE",
        },
      );
    ...
    try {
      ...
      await Promise.all(
        reviewsToDelete.map((r) =>
          fetch(`/api/products/${r.productId}/reviews/${r.id}`, {
            method: "DELETE",
          }),
        ),
      );
  ...
  const handleAddReply = async (review: ProductReview) => {
    ...
    try {
      ...
      const response = await fetch(
        `/api/products/${review.productId}/reviews/${review.id}/answers`,
        {
          method: "POST",
  ...
  const handleDeleteAnswer = async () => {
    ...
    try {
      ...
      const updateReviews = (currentReviews: ProductReview[]) => {
        return currentReviews.map((r) => {
          if (r.id === reviewId) {
            return {
              ...r,
              answers: r.answers?.filter((a) => a.id !== answerId),
            };
          }
          return r;
        });
      ...
      await fetch(
        `/api/products/${productId}/reviews/${reviewId}/answers/${answerId}`,
        {
          method: "DELETE",
        },
      );
  ...
  const stats = useMemo(() => {
    return {
      ...
      pending: reviews.filter((r) => !r.answers || r.answers.length === 0)
        .length, // Reviews without replies
      ...
      <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
        {[
          ...
          },
          {
            ...
            icon: CheckCircle,
            filter: "all",
          }, // Could add verified filter
        ].map((stat, i) => (
          <button
            key={i}
            onClick={() =>
              stat.filter !== "all" && setFilterRating(stat.filter)
            }
              ...
              >
                ...
                <div className="flex items-center gap-1 mb-3">
                  {[...Array(5)].map((_, i) => (
                    <Star
                      key={i}
                      className={`w-4 h-4 ${i < review.rating ? "text-neutral-900 fill-neutral-900" : "text-neutral-200 fill-neutral-200"}`}
                    />
                  ))}
                </div>
                  ...
                  <div className="space-y-4">
                    <div className="flex items-center gap-1">
                      {[...Array(5)].map((_, i) => (
                        <Star
                          key={i}
                          className={`w-5 h-5 ${i < selectedReview.rating ? "text-neutral-900 fill-neutral-900" : "text-neutral-200 fill-neutral-200"}`}
                        />
                      ))}
