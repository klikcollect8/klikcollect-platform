...
function CategoriesContent() {
  ...
  const handleDelete = async () => {
    if (confirmDialog.type === "bulk-delete") {
      ...
      try {
        setCategories((prev) =>
          prev.filter((c) => c.id !== confirmDialog.itemId),
        );
        if (viewingCategory?.id === confirmDialog.itemId)
          setViewingCategory(null);
        setConfirmDialog({ isOpen: false, type: "delete", itemId: null });

        const res = await fetch(`/api/categories?id=${confirmDialog.itemId}`, {
          method: "DELETE",
        });
        if (res.ok) {
  ...
  // --- Filtered Products ---
  const formFilteredProducts = products.filter(
    (p) =>
                  ...
                  <div className="flex items-center gap-2 text-sm text-neutral-500 mt-1">
                    ...
                    <span>
                      ...
                      {viewingCategory.createdAt
                        ? format(
                            new Date(viewingCategory.createdAt),
                            "MMMM d, yyyy",
                          )
                        : "Recently"}
                    </span>
              ...
              <div>
                ...
                {getCategoryProducts(viewingCategory.name).length > 0 ? (
                  <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-3 gap-4">
                    {getCategoryProducts(viewingCategory.name).map(
                      (product) => (
                        <div
                          key={product.id}
                          className="bg-white p-3 rounded-xl border border-neutral-100 flex items-center gap-3 shadow-sm hover:shadow-md transition-shadow"
                        >
                ...
                <div className="bg-white p-6 rounded-2xl border border-neutral-100">
                  <div className="flex items-center justify-between mb-4">
                    ...
                    >
                      ...
                      <ChevronRight
                        className={`w-3 h-3 transition-transform ${showProductSelector ? "rotate-90" : ""}`}
                      />
                    </button>
                  </div>
                  ...
                  {selectedProductIds.length > 0 && (
                    <div className="flex flex-wrap gap-2 mb-4">
                      {selectedProductsList.map((product) => (
                        <div
                          key={product.id}
                          className="flex items-center gap-2 bg-neutral-50 pl-2 pr-1 py-1 rounded-lg border border-neutral-200 text-xs text-neutral-700 animate-in fade-in zoom-in-95"
                        >
                          ...
                          formFilteredProducts.map((product) => {
                            ...
                            return (
                              ...
                              >
                                ...
                                <span
                                  className={`text-[10px] ${isSelected ? "text-neutral-400" : "text-neutral-400"}`}
                                >
                                  ${product.price}
                                </span>
                              </label>
...
export default function CategoriesPage() {
  return (
    <AccessControl requiredPermission="categories:view">
