...
import { getDeletedItems, restoreDeletedItem, permanentlyDeleteItem } from '@/lib/data';

export async function GET(request: NextRequest) {
  try {
    const searchParams = request.nextUrl.searchParams;
    const itemTypeParam = searchParams.get('type');
    const itemType = (itemTypeParam && ['product', 'review', 'question', 'answer', 'category', 'order'].includes(itemTypeParam)) 
      ? itemTypeParam as 'product' | 'review' | 'question' | 'answer' | 'category' | 'order'
      : undefined;
