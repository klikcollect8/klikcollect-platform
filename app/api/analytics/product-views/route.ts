...
export async function POST(request: NextRequest) {
  try {
    ...
    if (error) {
      ...
      return NextResponse.json({ success: true, note: 'Tracking table not configured' });
    }
...
export async function GET(request: NextRequest) {
  try {
    ...
    const views = data?.filter((a: { action: string }) => a.action === 'view').length || 0;
    const clicks = data?.filter((a: { action: string }) => a.action === 'click').length || 0;
    const purchases = data?.filter((a: { action: string }) => a.action === 'purchase').length || 0;
