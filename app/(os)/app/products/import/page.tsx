...
export default function ImportDryRunPage() {
  ...
  async function commitImport() {
    if (!result?.summary.valid || !vendorId) return;
    setBusy(true);
    setError("");
    try {
      track(
        "os.import_commit_clicked",
      ...
      const res = await fetch("/api/products/import/commit", {
        method: "POST",
        headers: { "Content-Type": "application/json" },
        body: JSON.stringify({ csv, vendorId }),
      });
      const json = await res.json();
      if (!res.ok) {
  ...
  return (
    <ModuleShell
      title="Bulk import"
      description="CSV dry-run against V1 categories (name, category, price, stock), then commit to your store catalogue."
      live
      actions={
        <Link
          href="/app/products"
          ...
          {result ? (
            <div className="space-y-6">
                  ...
                  {result.preview.map((row) => (
                    ...
                    >
                      ...
                      <span className="text-neutral-500">
                        {row.category} · {formatKesMajor(row.priceMajor)} ·
                        stock {row.stock}
                      </span>
                    </div>
                  ))}
              ...
              {result.summary.valid > 0 && committed === null ? (
                <div className="flex justify-center">
                  ...
                  >
                    {busy
                      ? "Committing…"
                      : `Commit ${result.summary.valid} valid rows`}
                  </button>
                </div>
              ) : null}
              ...
              <p className="text-center text-sm text-neutral-500">
                {vendorId
                  ? `Writes to your store · ${vendorId}`
                  : "Sign in with a vendor membership to commit."}
              </p>
            </div>
          ) : null}
