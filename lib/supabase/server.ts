...
export function createAdminClient() {
  ...
  if (!serviceRoleKey) {
    console.warn('[createAdminClient] SUPABASE_SERVICE_ROLE_KEY not found in environment variables. Falling back to anon key (may be limited by RLS).')
    return null
  }
...
export async function createClient() {
  ...
  if (!env) {
    ...
    const createMockQuery = () => {
      ...
      const mockQuery: any = Object.assign(
  ...
  try {
    ...
    const createMockQueryFallback = () => {
      ...
      const mockQueryFallback: any = Object.assign(
        Promise.resolve(result),
        {
