...
export default function CheckoutWizard() {
  ...
  const payOptions = availablePayMethods(
            ...
            <form id="kc-checkout-wizard" onSubmit={handleSubmit(onSubmit)}>
              ...
              {step === "payment" ? (
                <div>
                  {heading(
                    "How you’ll pay",
                    "M-Pesa, cards, wallets, bank, or USSD — pick one rail to continue.",
                  )}
                  {!paymentsAvailable ? (
                    ...
                    <div className="mt-8 space-y-6">
                      {(["paystack", "stripe"] as const).map((group) => {
                        const groupMethods = payOptions.filter(
                          (m) => m.group === group,
                        );
                                  ...
                                  <li key={m.id}>
                                    ...
                                    >
                                      ...
                                      <span className="min-w-0 flex-1">
                                        ...
                                        <span className="mt-0.5 block text-[13px] text-black/40">
                                          {m.description}
                                        </span>
                                      </span>
                                    </button>
                      ...
                      {isMpesaPayMethod(payMethod) ? (
                        <div className="border border-black/10 bg-white/50 px-4 py-4">
                          <p className="mb-3 text-[12px] text-black/40">
                            M-Pesa number for the STK push
                          </p>
                          <PhoneField
                      ...
                      {isStripePayMethod(payMethod) ? (
                        <p className="text-[13px] text-black/40">
                          You’ll finish on Stripe Checkout — card, Apple Pay,
                          Google Pay, or Link when available.
                        </p>
                      ) : null}
                      {isPaystackHostedMethod(payMethod) ? (
                        <p className="text-[13px] text-black/40">
                          You’ll finish on Paystack’s secure payment page.
                        </p>
                      ) : null}
                    </div>
                  )}
