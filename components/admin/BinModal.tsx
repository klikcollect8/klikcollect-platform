...
export default function BinModal({
      ...
      if (response.ok) {
        ...
        if (itemType === "answer") {
          if (filterByReviewId) {
            data = data.filter(
              (item: DeletedItem) =>
                item.itemData?.review_id || item.itemData?.reviewId,
            );
          } else if (filterByQuestionId) {
            data = data.filter(
              (item: DeletedItem) => item.itemData?.question_id,
            );
          }
        }
    ...
    try {
      ...
      const options =
        action === "restore"
          ? {
              method: "POST",
              body: JSON.stringify({ deletedItemId: confirmDialog.itemId }),
            }
          : { method: "DELETE" };
      ...
      setDeletedItems((prev) =>
        prev.filter((i) => i.id !== confirmDialog.itemId),
      );
  ...
  const handleBulkAction = async (action: "restore" | "delete") => {
    ...
    try {
      ...
      if (failCount === 0) {
        showToast(
          `${successCount} items ${action === "restore" ? "restored" : "deleted"} successfully`,
          "success",
        );
      } else {
  ...
  function getItemDisplayName(item: DeletedItem) {
    ...
    switch (item.itemType) {
      ...
      case "review":
        return (
          data.title ||
          (data.comment ? data.comment.substring(0, 40) + "..." : "Review")
        );
      case "question":
        return data.question
          ? data.question.substring(0, 40) + "..."
          : "Question";
      case "answer":
        return data.answer ? data.answer.substring(0, 40) + "..." : "Answer";
      case "category":
        return data.name || "Category";
      case "order":
        return `Order #${data.orderNumber || data.order_number || item.itemId}`;
      default:
        return "Unknown Item";
    }
  }
  ...
  const displayTitle =
    title ||
    `${itemType.charAt(0).toUpperCase() + itemType.slice(1)} Recycle Bin`;
  ...
  return (
    <>
              ...
              <div className="flex flex-col items-center justify-center h-full text-center py-12">
                ...
                <p className="text-neutral-500 text-sm max-w-xs mx-auto">
                  {searchQuery
                    ...
                    : "Deleted items will appear here. They are automatically removed after 30 days."}
                </p>
              </div>
            ) : (
                  ...
                  return (
                    <div
                      ...
                      className={`group relative bg-white rounded-2xl border transition-all duration-300 hover:shadow-lg hover:-translate-y-0.5 ${
                        isSelected
                          ? "border-neutral-300 ring-1 ring-neutral-200 shadow-md"
      ...
      <ConfirmDialog
        ...
        confirmText={
          confirmDialog.type.includes("restore") ? "Restore" : "Delete Forever"
        }
        cancelText="Cancel"
        ...
        onConfirm={() => {
          if (confirmDialog.type.includes("bulk")) {
            handleBulkAction(
              confirmDialog.type.includes("restore") ? "restore" : "delete",
            );
          } else {
            handleSingleAction(
