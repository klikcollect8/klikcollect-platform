  ...
  const getSpecifications = () => {
    ...
    if (product.category === 'Electronics') {
      return [
        ...
        { label: 'Color', value: product.variations?.find(v => v.name === 'Color')?.selected || 'N/A' },
    ...
    if (product.category === 'Sports & Outdoors') {
      return [
        ...
        { label: 'Color', value: product.variations?.find(v => v.name === 'Color')?.selected || 'N/A' },
        { label: 'Size', value: product.variations?.find(v => v.name === 'Size')?.selected || 'One Size' },
    ...
    <div>
      ...
      <div className="space-y-0 border border-gray-100 rounded-xl overflow-hidden">
        {specifications.map((spec, idx) => (
          ...
          >
            <dt className="w-1/3 text-sm font-medium text-gray-900 pr-4">{spec.label}</dt>
